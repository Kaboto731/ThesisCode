...
import keras
import h5py
def nanglesposcirlce(timesteps,epo,examples,numRNNneurons,savennfile,loadnnfile,m1a,mua,nangles):
    ...
    if (loadnnfile == False):
        ...
        [mymodel,mymodel2]= buildneuralnet(timesteps,numRNNneurons+4,dropped,numRNNneurons,m1a,mua)
        ...
        mycallbacks = [keras.callbacks.EarlyStopping(monitor='val_loss',min_delta=0,patience=50,verbose=1, mode='auto'),mc]
    ...
    mymodel.summary()
    mymodel2.summary()
    #after training nangles directions with the same go signal we want to get a slice of the degree
    ...
    for i in range(nangles):
        for z in range(numRNNneurons):
            myinner[i,z] = np.matmul(np.transpose(PDvec[:,z]),theta2vec[:,i])
            myexp=-(myinner[i,z])
            v[i,z]= np.exp(myexp)
    #making the onehot encoding appended
        vslicepro[i,:,0:numRNNneurons+3]= np.append(v[i],onehotpro)
        handmatpro = np.array(matpro)
        pcount=pcount+1
        vslicemid[i,:,0:numRNNneurons+3]=np.append(v[i],onehotmid)
        handmatmid = np.array(matmid)
        mcount=mcount+1
        vslicesup[i,:,0:numRNNneurons+3] = np.append(v[i],onehotsup)
        ...
        for j in range(0,numRNNneurons):
                ...
                #have greater variance
                noise[i,j] = random.gauss(mu,noisesigma[i,j])
                vslicesup[i,:,j]= vslicesup[i,:,j]+noise[i,j]
                vslicemid[i,:,j]= vslicemid[i,:,j]+noise[i,j]
                vslicepro[i,:,j]= vslicepro[i,:,j]+noise[i,j]
        #our fixed go signal starting
        goat[i]=30  
    ...
    return timer,history,theta2,vslicereshapepro,reshapedhandmatpro,proheatinput,proheatoutput,proforce,vslicereshapemid,reshapedhandmatmid,midheatinput,midheatoutput,midforce,vslicereshapesup,reshapedhandmatsup,supheatinput,supheatoutput,supforce,holdperiod
