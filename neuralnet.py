...
def myeslu(x):
    ...
    #  cond22 = tf.cast(tf.math.less_equal(x, 0.0), tf.float32)
      ...
      a = tf.math.multiply_no_nan(cond1, lalpha/lbeta*(K.sqrt(x)+1))#lalpha/lbeta*K.pow(x,lbeta)+lalpha
      b = tf.math.multiply_no_nan(cond2, lalpha/lbeta*(x+1))
      d = tf.math.multiply_no_nan(cond4, lalpha/lbeta*K.exp(x))#lalpha*K.exp(x)
    ...
    #   print(sess.run(f, feed_dict={x: [y]}))    
      ...
      return f#,grad
get_custom_objects().update({'myeslu': Activation(myeslu)})
...
def buildneuralnet(time,channels,dropped,numRNNneurons,m1a,mua):
    inputlayer1 = Input(shape=(time,channels))
    #units was channels, now 30
    dotmatinput = Input(shape=(6,2))
    model2 = buildneuralnet2(time,channels,dropped,numRNNneurons,m1a,mua)
    ...
    return model,model2
def buildneuralnet2(time,channels,dropped,numRNNneurons,m1a,mua):
    actualinput = Input(shape=(time,channels))
    #units was channels, now 30
    m1context = layers.SimpleRNN(units=numRNNneurons,input_shape=(time,channels),activation = myeslu,return_sequences=True,dropout=dropped, activity_regularizer=regularizers.l2(m1a))(actualinput)#kernel_regularizer = regularizers.l2(0.5), activity_regularizer=regularizers.l1(0.5)
